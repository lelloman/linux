...
static int test_no_invasive_cgroup_shrink(const char *root)
{
	...
	/* Allocate 10x memory.max to push test_group1 memory into zswap */
	if (cg_run(wb_group, allocate_bytes, (void *)MB(10)))
		goto out;

	/* Verify that only zswapped memory from group 1 has been written back */
	if (get_zswap_written_back_pages(&written_back_after))
		goto out;
	group_wb = cg_read_key_long(wb_group, "memory.stat", "zswap_wb");
	if (group_wb < 0)
		goto out;
	if (cg_read_key_long(control_group, "memory.stat", "zswap_wb") != 0)
		goto out;
	if (written_back_after - written_back_before == group_wb)
		ret = KSFT_PASS;
out:
